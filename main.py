...
def main():
    ...
    parser.add_argument(
        "--prop-path",
        "-p",
        help="(Optional) Path to prop file (.blend) or path \
                            to directory containing props. Default is BlendGen/props/",
    ...
    parser.add_argument(
        "--n-instances",
        "-i",
        help="Number of instances of the model in each render. \
                        Maximum is 31. Default is 1.",
        default=1,
    )
    ...
    # Launch Blender
    command = f"blender --background --python {MAIN_BLENDER_PATH} \
        -- {prop_path} {save_dir_path} {n_images} {n_instances}"
